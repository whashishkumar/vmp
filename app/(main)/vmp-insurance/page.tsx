...
export default async function page() {
  const resp = await ServicesPageEndPoint.getVmpInsurance();
  const { about, cta_section, hero, insurance, investment, partnership } = resp || {};
  return (
    <div>
      <InnerBanner bannerInfo={hero} className="bg-color" />
      <HeroSection data={about} />
      <InsuranceSection data={insurance} />
      <InvestmentSolutions data={investment} />
      <PartnerNetworkSection data={partnership} />
      <InsuranceHeroLight data={cta_section} />
    </div>
  );
}
